...
- **Watchlist** — single fundamentals-rich table over every holding + every user-added watchlist item. Filter chips (All / Held / Watch); "Upcoming events" panel at the top aggregating earnings + ex-dividend dates in the next 30 days. Per row: symbol + name + sector/country badges, last price, day %, market cap, P/E (TTM + Fwd), EPS TTM, dividend yield, revenue YoY %, EPS YoY %, 52-week range mini-bar, next earnings date (with EPS estimate), next ex-dividend date (with amount), Held / Watch badge plus quantity + market value + unrealized % for holdings. An "+ Add ticker" modal proxies Finnhub `/search` for typeahead and accepts an optional Yahoo symbol + free-text notes per item. Row menu opens the instrument-detail modal for holdings, edits notes / Yahoo fallback for watchlist items, removes watchlist entries, and links to the company's website. When the `finnhub_api_key` addon option is unset the tab still renders (prices via Yahoo fallback only) with a banner explaining how to enable fundamentals.
...
- **Fundamentals (Finnhub)** — primary fundamentals source for US stocks. Pulls `/quote`, `/stock/metric?metric=all`, `/stock/profile2`, `/calendar/earnings`, `/stock/dividend`, and `/search`. Requires a free API key in the addon option `finnhub_api_key`; when unset the service is in disabled mode (every call resolves to `null` and the Yahoo fallback below takes over). Per-endpoint TTL caching in `/data/fundamentals-cache.json`: quote 15 min, metric 24 h, profile 7 d, earnings 6 h, dividend 24 h. Failures and empty responses are negative-cached for 1 h so symbols outside the free tier's non-US coverage don't hammer the upstream. Per-symbol fan-out is throttled to 5 concurrent calls to stay under the 60 req/min free-tier ceiling. Free tier is hard-locked to US — every non-US symbol 403s, which is why we also wire the Yahoo fallback below.
- **Fundamentals (Yahoo quoteSummary)** — fallback for symbols Finnhub doesn't cover (i.e. virtually every non-US holding). Hits Yahoo's unofficial v10 quoteSummary endpoint with the `summaryDetail`, `defaultKeyStatistics`, `financialData`, `calendarEvents`, `price`, and `assetProfile` modules. No auth needed but Yahoo gates the endpoint behind a `crumb` cookie — the service performs the handshake (`fc.yahoo.com` → `getcrumb`) lazily on first call and refreshes it once on 401. Cached to `/data/yahoo-fundamentals-cache.json` with a 12-hour TTL (1 h negative cache for transient failures).
...
- `/data/fundamentals-cache.json` — Finnhub fundamentals cache (derived)
...
- `POST /api/watchlist` — add a watchlist item `{ finnhubSymbol, yahooSymbol?, displayName?, notes? }`; idempotent on `finnhubSymbol`
- `PATCH /api/watchlist/:id` — update notes / symbols on a watchlist item
- `DELETE /api/watchlist/:id` — remove a watchlist item
- `GET /api/research` — merged feed (every holding + every watchlist item) enriched with Finnhub quote / metrics / profile / next earnings / next ex-dividend, plus a 30-day upcoming-events list
- `POST /api/research/refresh` — invalidates the Finnhub quote cache and rebuilds the feed (slower fundamentals / profile / earnings caches keep their longer TTLs)
- `GET /api/research/search?q=` — proxies Finnhub `/search` for the Add-ticker modal's typeahead
