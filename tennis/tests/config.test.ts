...
it('drops legacy custom dates on load and save while preserving future weekday settings', async () => {
  ...
  try {
    ...
    expect(options).not.toHaveProperty('scan_dates');
    expect(options.seb_future_weekdays).toEqual([1,5]);
    expect(scanDatePlan(options,new Date('2026-09-09T12:00:00Z')).near).toHaveLength(7);
    saveOptions({...options,...{scan_dates:['2027-02-10']}});
    expect(JSON.parse(readFileSync(join(dir,'config.json'),'utf8'))).not.toHaveProperty('scan_dates');
