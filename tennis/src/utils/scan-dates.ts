...
export function scanDateBounds(now = new Date(), months = 6) {
  ...
  end.setUTCDate(1); end.setUTCMonth(end.getUTCMonth()+months+1); end.setUTCDate(0);
...
export function scanDatePlan(options: {scan_dates: string[]; seb_future_weekdays: number[]; seb_future_months?: number}, now = new Date()) {
  const bounds = scanDateBounds(now, options.seb_future_months ?? 6);
