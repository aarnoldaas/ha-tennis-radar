...
export class HomeAssistantNotifier {
  ...
  async sendBookingReminder(
    ...
    const title = `\u{1F3BE} Booking in ${threshold.label}`;
    const hoursRounded = Math.round(hoursUntil);
    const message =
      `${booking.courtName} on ${booking.date} at ${booking.startTime}\u2013${booking.endTime} ` +
      `(${booking.provider}) \u2014 ~${hoursRounded}h away`;
    const id = `tennis_booking_reminder_${threshold.name}_${booking.provider}_${booking.date}_${booking.startTime}`;

    try {
      await this.sendPersistentNotification(message, title, id);
      console.log(`[Notifier] Booking reminder (${threshold.name}) sent: ${booking.provider} ${booking.date} ${booking.startTime}`);
    } catch (err) {
      console.error('[Notifier] Failed to send booking reminder:', err);
    }

    if (deviceId) {
      try {
        await this.sendMobilePush(deviceId, title, message);
        console.log(`[Notifier] Booking reminder push sent to ${deviceId}`);
      } catch (err) {
        console.error('[Notifier] Failed to send booking reminder push:', err);
      }
    }
  }
