...
beforeEach(() => {dir = mkdtempSync(join(tmpdir(),'tennis-cart-test-')); vi.useFakeTimers();vi.setSystemTime(new Date('2026-09-09T12:00:00Z')); vi.spyOn(SebProvider.prototype,'getBookings').mockResolvedValue([]);});
...
it('adds only the highest available court, for the configured duration, after the action is clicked', async () => {
  ...
  vi.spyOn(SebProvider.prototype,'checkoutWithCredit').mockImplementation(async (_code,_slot,_limit,save) => {save(36);return 36;});
  ...
  expect(actions.list()[0]).toMatchObject({state:'paid',cartCode:'cart-123',bookingsUrl:SEB_BOOKINGS_URL});
  await actions.handle(notification.action);
