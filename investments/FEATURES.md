...
- **Fundamentals (Finnhub)** — drives the Watchlist tab. Pulls `/quote`, `/stock/metric?metric=all`, `/stock/profile2`, `/calendar/earnings`, `/stock/dividend`, and `/search`. Requires a free API key in the addon option `finnhub_api_key`; when unset the service is in disabled mode (every call resolves to `null` and the UI degrades to "—"). Per-endpoint TTL caching in `/data/fundamentals-cache.json`: quote 15 min, metric 24 h, profile 7 d, earnings 6 h, dividend 24 h. Failures and empty responses are negative-cached for 1 h so symbols outside the free tier's non-US coverage don't hammer the upstream. Per-symbol fan-out is throttled to 5 concurrent calls to stay under the 60 req/min free-tier ceiling.
