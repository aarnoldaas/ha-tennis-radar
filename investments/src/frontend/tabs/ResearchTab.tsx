...
import { currencyFmt, money, num, pct, pnlColor, signedPct } from '../lib/format';
...
export function ResearchTab({
  ...
  const sortedRows = useMemo(() => {
    if (!data) return [];
    let rows = [...data.rows];
    if (filter === 'held') rows = rows.filter(r => r.kind !== 'watchlist');
    if (filter === 'watch') rows = rows.filter(r => r.kind !== 'holding');
    return rows.sort((a, b) => {
      const av = a.valueEur ?? -1;
      const bv = b.valueEur ?? -1;
  ...
  return (
    <Stack gap="md">
      ...
      <UpcomingPanel upcoming={data.upcoming} open={eventsOpen} onToggle={() => setEventsOpen(o => !o)} />
...
function FilterChip({
  ...
  return (
    <Button size="xs" variant={active ? 'filled' : 'default'} onClick={onClick} radius="xl">
      {label}{' '}
          ...
          {upcoming.map((e, idx) => (
            <Group key={`${e.rowId}:${e.kind}:${idx}`} justify="space-between" wrap="nowrap">
              <Group gap="xs" wrap="nowrap">
                <Badge size="xs" variant="light" color={e.kind === 'earnings' ? 'yellow' : 'teal'}>
  ...
  return (
    <Table.Tr>
      ...
      <Table.Td ta="right" className="lh-mono">
        {row.price != null && row.priceCurrency ? currencyFmt(row.price, row.priceCurrency) : '—'}
      </Table.Td>
      <Table.Td ta="right" className="lh-mono">
      ...
      <Table.Td>
        <DateCell
          ...
          extra={
            row.nextEarnings?.epsEstimate != null ? `est ${row.nextEarnings.epsEstimate.toFixed(2)}` : null
          }
        />
      </Table.Td>
      <Table.Td>
      ...
      <Table.Td>
        <Stack gap={2}>
          ...
          {row.quantity != null && (
            <Text size="xs" c="dimmed" className="lh-mono">
              {num(row.quantity)} · {money(row.valueEur)}
            </Text>
          )}
          {row.gainPct != null && (
            <Text size="xs" c={pnlColor(row.gainPct)} className="lh-mono">
      ...
      <Table.Td>
        <RowMenu row={row} onOpenInstrument={openHoldingDetail} onEdit={() => onEdit(row)} onRemoved={onRemoved} />
      </Table.Td>
    </Table.Tr>
  );
}
...
function RowMenu({
  ...
  return (
    <Menu shadow="md" position="bottom-end" withinPortal>
      ...
      <Menu.Dropdown>
        {onOpenInstrument && <Menu.Item onClick={onOpenInstrument}>Open instrument detail</Menu.Item>}
...
function AddTickerModal({
  ...
  const [verified, setVerified] = useState<YahooVerifyResult | null>(null);
      ...
      <Stack gap="sm">
        ...
        {verified && (
          <Alert color="teal" variant="light" p="xs">
            ...
            <Text size="xs" c="dimmed" className="lh-mono">
              {verified.symbol} · {verified.exchangeName ?? '?'} · {verified.currency ?? '?'}
  ...
  return (
    <Modal opened={!!row} onClose={onClose} title={`Edit ${row.symbol ?? row.displayName}`}>
      <Stack gap="sm">
        ...
        <TextInput label="Yahoo symbol" value={symbol} onChange={e => setSymbol(e.currentTarget.value)} />
