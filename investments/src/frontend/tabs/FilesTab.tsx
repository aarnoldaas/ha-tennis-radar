...
const BROKER_DIRS: Record<string, string> = {
  'Investments/swedbank': BROKERS.swedbank.label,
  'Investments/interactive-brokers': BROKERS['interactive-brokers'].label,
};
...
const PROTECTED: Record<string, string> = {
  ...
  'price-cache.json': 'Yahoo price cache. Will be re-fetched on next refresh.',
};
...
function groupByDir(files: DataFileEntry[]): GroupedFiles[] {
  ...
  return dirs.map(d => ({ dir: d, files: (map.get(d) ?? []).sort((a, b) => a.path.localeCompare(b.path)) }));
}

function dirLabel(dir: string): string {
  return BROKER_DIRS[dir] ? `${BROKER_DIRS[dir]} (${dir}/)` : dir + '/';
}
...
export function FilesTab() {
  ...
  const [uploadDir, setUploadDir] = useState<string>('Investments/swedbank');
  ...
  const knownDirs = useMemo(() => {
    const set = new Set<string>(Object.keys(BROKER_DIRS));
    for (const f of files) {
      const d = dirOf(f.path);
  ...
  const dirOptions = useMemo(
    () => [
      ...knownDirs.map(d => ({ value: d, label: dirLabel(d) })),
      { value: '', label: '/ (root)' },
  ...
  const effectiveUploadDir = uploadDir === '__custom__' ? customDir.trim().replace(/^\/+|\/+$/g, '') : uploadDir;
  ...
  const handleUpload = async (selected: File[]) => {
    ...
    setMessage(null);
    try {
      const result = await api.uploadFiles(effectiveUploadDir, selected);
      if (result.ok) {
        setMessage({ type: 'success', text: `Uploaded ${result.uploaded?.length ?? 0} file(s).` });
        await load();
      } else {
  ...
  const handleDelete = async (path: string) => {
    ...
    const prompt = warning ? `Delete ${path}?\n\n${warning}` : `Delete ${path}?`;
    ...
    <Stack gap="md">
      ...
      <Card padding="md" withBorder>
        ...
        <Text size="xs" c="dimmed" mt="xs">
          Files upload directly to the chosen folder under <code>/data</code>. Re-uploads with overlapping date
          ranges are deduplicated automatically on the next portfolio rebuild.
        </Text>
      </Card>
        ...
        {files.length === 0 ? (
          ...
          grouped.map(group => (
            <div key={group.dir} style={{ marginBottom: 16 }}>
              ...
              >
                {group.dir === ROOT_DIR ? '/' : dirLabel(group.dir)}
              </Text>
              <Table highlightOnHover horizontalSpacing="xs" verticalSpacing={6} style={{ tableLayout: 'fixed' }}>
                <Table.Tbody>
                  {group.files.map(f => {
                      ...
                      <Table.Tr key={f.path}>
                        ...
                        <Table.Td style={{ width: 160, textAlign: 'right' }}>
                          <Group gap={4} justify="flex-end" wrap="nowrap">
                            <Anchor href={api.fileDownloadUrl(f.path)} download={name} size="xs">
                              Download
                            </Anchor>
                            <Button size="compact-xs" variant="subtle" color="red" onClick={() => handleDelete(f.path)}>
                              Delete
                            </Button>
                          </Group>
                        </Table.Td>
